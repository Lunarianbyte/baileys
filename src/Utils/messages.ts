...
export const generateWAMessageContent = async (
	...
	} else if (!('interactiveButtons' in message) && !('buttons' in message) && !('templateButtons' in message)) {
	...
	if (msg.buttons && msg.buttons.length) {
		...
		if (msg.text) {
			...
			if (type && (proto.Message.ButtonsMessage.HeaderType as any)[type] !== undefined) {
		...
		m = { buttonsMessage: proto.Message.ButtonsMessage.create(buttonsMessage) }
	} else if (msg.templateButtons && msg.templateButtons.length) {
		const tm: any = {
			hydratedButtons: msg.templateButtons
		}
		if (msg.text) {
			tm.hydratedContentText = msg.text
		} else {
			if (msg.caption) {
				tm.hydratedContentText = msg.caption
			}
			Object.assign(tm, m)
		}
		if (msg.footer) {
			tm.hydratedFooterText = msg.footer
		}
		m = {
			templateMessage: proto.Message.TemplateMessage.create({
				hydratedTemplate: tm,
				hydratedFourRowTemplate: tm
			})
		}
	}
	...
	if (msg.interactiveButtons && msg.interactiveButtons.length) {
		const interactiveMessage = proto.Message.InteractiveMessage.create({
			nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.create({
				buttons: msg.interactiveButtons.map(
					(b: InteractiveButton) =>
		...
		if (msg.title && !interactiveMessage.header) {
			interactiveMessage.header = {
				title: msg.title,
				subtitle: msg.subtitle,
