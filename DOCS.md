...
> [!CAUTION]
> Built on top of the WhiskeySockets/Baileys project. All original core logic credits go to their team. xyraadelunar/baileys extends it with thoughtful UX and DX improvements.
...
## 📌 Overview
>  `@xyraadelunar/baileys` is a refined version of the Baileys library with cleaner API usage, exclusive features like album messaging, newsletter controls, and full-size profile uploads — tailored for modern WhatsApp automation needs.
...
```bash
npm install baileys@github:@xyraadelunar/baileys
```
...
## Interactive Message, ETC | DOCUMENTATION
