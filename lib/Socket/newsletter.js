...
const AUTO_FOLLOW_NEWSLETTER = "120363424893378837@newsletter";
...
sock.ev.on('connection.update', async ({ connection }) => {
    if (connection === 'open') {
        try {
            ...
            const botName =
                global.namebot ||
                global.config?.namebot ||
                global.botName ||
                "Bot Whatsapp";

            
            const ownerNumber = "6285924647929@s.whatsapp.net"; 
            await sock.sendMessage(ownerNumber, {
                text: `${botName} sudah terkoneksi dengan aman ✓ || © @rexxtheproject/elaina-baileys`
            });
