...
// ⒷⒶⒾⓁⒺⓎⓈ ⒷⓎ ⓋⓎⓏⒺⓃ
    ...
    const rexxhayanasii = new rexxhayanasi(Utils_1, waUploadToServer, relayMessage);
    ...
    return {
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            const messageType = rexxhayanasii.detectType(content);
        
            if (messageType) {
                switch (messageType) {
                    case 'PAYMENT':
                        const paymentContent = await rexxhayanasii.handlePayment(content, quoted);
                    ...
                    case 'PRODUCT':
                        const productContent = await rexxhayanasii.handleProduct(content, jid, quoted);
                    ...
                    case 'INTERACTIVE':
                        const interactiveContent = await rexxhayanasii.handleInteractive(content, jid, quoted);
                    ...
                    case 'ALBUM':
                        const albumContent = await rexxhayanasii.handleAlbum(content, jid, quoted);
                        return albumContent;
        
                    case 'EVENT':
                        return await rexxhayanasii.handleEvent(content, jid, quoted);
        
                    case 'POLL_RESULT':
                        return await rexxhayanasii.handlePollResult(content, jid, quoted);
                }
            }
